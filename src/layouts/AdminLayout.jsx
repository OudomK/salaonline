...
import { useState } from "react";
import { Outlet, Link, useLocation } from "react-router-dom";
import { LayoutDashboard, Users, Menu, X, LogOut, FileCheck, BookOpen, BookCheck, Settings } from "lucide-react";
...
export default function AdminLayout() {
  ...
  const menuItems = [
    ...
    { path: "/admin/students", label: "Students", icon: <Users size={20}/> },
    { path: "/admin/requests", label: "Exam Requests", icon: <FileCheck size={20}/> },
    { path: "/admin/courses", label: "Course Manager", icon: <BookOpen size={20}/> },
    { path: "/admin/homework", label: "Homework", icon: <BookCheck size={20}/> },
  ...
  return (
    ...
    <div className="flex h-screen bg-gray-100 overflow-hidden">
      ...
      <div className="md:hidden fixed top-0 w-full bg-[#1a1f2c] text-white p-4 flex justify-between items-center z-30 shadow-md h-16">
         <span className="font-bold font-khmer-os-battambang tracking-wide">SALA ADMIN</span>
         <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="p-1 rounded-md hover:bg-white/10">
