...
import { ChevronLeft, MoreHorizontal, Play, Lock, FileBadge, CheckCircle, Clock, BookOpen, FileText } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 pb-32 md:pb-10 font-khmer-os-battambang">
      ...
      <div className="sticky top-0 z-30 flex justify-between items-center px-4 py-3 bg-white/80 backdrop-blur-md border-b border-gray-100 md:hidden">
        ...
        <h1 className="text-base font-bold text-gray-900 line-clamp-1">{courseDetail.title}</h1>
      ...
      <div className="max-w-7xl mx-auto md:p-6 md:grid md:grid-cols-3 md:gap-8">
          ...
          <div className="md:col-span-2 space-y-6">
             ...
             <div className="w-full aspect-video md:rounded-[24px] overflow-hidden shadow-xl shadow-blue-50 relative group bg-black sticky top-0 md:static z-20">
                <img src={courseDetail.coverImage} className="w-full h-full object-cover opacity-80 group-hover:opacity-60 transition-opacity" alt="Cover" />
                                    ...
                                    >
                                        <div className="flex gap-4 items-center">
                                            <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm shrink-0 ${
                                                isActive ? "bg-[#00B4F6] text-white" : "bg-gray-100 text-gray-400"
                                            }`}>
                                                {String(index + 1).padStart(2, '0')}
                                            </div>
                                            <div className="min-w-0">
                                                <h4 className={`font-bold text-sm truncate ${isActive ? "text-[#00B4F6]" : (lesson.isLocked ? "text-gray-400" : "text-gray-700")}`}>
                                                    {lesson.title}
                                                </h4>
                                        ...
                                        <div className="text-gray-300 pl-2">
                                            {lesson.isLocked ? <Lock size={16} /> : isActive ? (
           ...
           <div className="fixed bottom-[70px] left-0 right-0 p-4 bg-white/90 backdrop-blur-sm border-t border-gray-100 md:static md:bg-transparent md:border-0 md:p-0 z-20">
                <div className="flex gap-3 max-w-7xl mx-auto">
                    ...
                    <Button 
                        ...
                        className={`flex-1 h-12 rounded-xl font-bold text-base text-white shadow-lg shadow-blue-200 ${
                            examStatus === "pending" ? "bg-yellow-500 hover:bg-yellow-600" : "bg-[#00B4F6] hover:bg-[#009bd1]"
                        }`}
                    >
                        {examStatus === "pending" ? (
