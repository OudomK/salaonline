...
import { Search, Plus, Video, Edit, Trash2, PlayCircle, BookOpen, ChevronLeft, ChevronRight, Loader2, Eye, EyeOff } from "lucide-react";
import { useAdminCourses, useCreateCourse, useUpdateCourse, useDeleteCourse, usePublishCourse, useHideCourse } from "@/hooks/api";
...
export default function CourseManager() {
   ...
   return (
      <div className="space-y-6 animate-fade-in-up pb-20">
            ...
            <Table>
               <TableHeader>
                  <TableRow className="bg-gray-50 border-b border-gray-100 hover:bg-gray-50">
                     <TableHead className="px-6 py-4 text-xs font-extrabold text-gray-400 uppercase tracking-wider">THUMBNAIL</TableHead>
                     <TableHead className="px-6 py-4 text-xs font-extrabold text-gray-400 uppercase tracking-wider">COURSE NAME</TableHead>
                     <TableHead className="px-6 py-4 text-xs font-extrabold text-gray-400 uppercase tracking-wider">CATEGORY</TableHead>
                     <TableHead className="px-6 py-4 text-xs font-extrabold text-gray-400 uppercase tracking-wider">STATUS</TableHead>

                     <TableHead className="px-6 py-4 text-center text-xs font-extrabold text-gray-400 uppercase tracking-wider">ACTION</TableHead>
                  </TableRow>
               </TableHeader>
               <TableBody>
                  {isLoadingCourses ? (
                           ...
                           <TableCell className="px-6 py-4">
                              <div className="flex items-center gap-3">
                                 ...
                                 <div>
                                    ...
                                    <p className="text-xs text-gray-400">{course.teacher_name || course.teacher?.username || "No Teacher"}</p>
                                 </div>
                              </div>
                           </TableCell>
                           <TableCell className="px-6 py-4">
                              <span className={`px-3 py-1 rounded-lg text-xs font-bold ${getCategoryColor(course.category_id || 1)}`}>
                                 {getCategoryName(course.category_id || 1)}
                              </span>
                           </TableCell>
                           <TableCell className="px-6 py-4">
                              <span className="text-sm font-bold text-gray-700 bg-gray-100 px-3 py-1 rounded-full">{course.status || "N/A"}</span>
                           </TableCell>

                           <TableCell className="px-6 py-4 text-center">
                              <div className="flex justify-center gap-2">
                                 <button onClick={() => handleToggleStatus(course)} className={`p-2 rounded-lg transition-all ${course.status === 'published' ? 'bg-orange-50 text-orange-500 hover:bg-orange-500 hover:text-white' : 'bg-green-50 text-green-500 hover:bg-green-500 hover:text-white'}`} title={course.status === 'published' ? 'Hide Course' : 'Publish Course'}>
            ...
            {totalItems > 0 && (
               <div className="flex flex-col md:flex-row justify-between items-center p-4 border-t border-gray-100 bg-gray-50/50 gap-4">
                  <span className="text-xs font-bold text-gray-500 font-khmer-os-battambang">
                     បង្ហាញ {indexOfFirstItem + 1} ដល់ {indexOfLastItem} នៃ {totalItems} វគ្គសិក្សា
                  </span>
         ...
         <CrateForm isModalOpen={isModalOpen} setIsModalOpen={setIsModalOpen} currentCourse={currentCourse} />
